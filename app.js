...
async function shareMonthReport() {
  ...
  let blob, nome;
  try {
    const { doc } = _shareRelatorioDoc(_monthShareModel(off));
    blob = doc.output('blob');
    nome = _pdfNomeArquivo(mLabel);
  } catch (e) { console.error(e); gdToast('Não foi possível gerar o relatório.', { type: 'error' }); return; }
  const file = new File([blob], nome, { type: 'application/pdf' });
  if (navigator.share && navigator.canShare && navigator.canShare({ files: [file] })) {
    try { await navigator.share({ files: [file], title: `Avenco — ${mLabel}` }); } catch (e) {}
  } else {
    const url = URL.createObjectURL(blob);
  ...
  root.innerHTML = `
    ...
    <div class="sgrp">
      ...
      <div class="srow srow-muted">
        ...
        <div class="srow-body"><div class="srow-label">Versão</div><div class="srow-value">Avenco v100</div></div>
      </div>
      <div class="sdivider"></div>
      <div class="srow srow-muted">
